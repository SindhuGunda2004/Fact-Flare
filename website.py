...
# This to integrate this model to the website
app = Flask(__name__)
...
train_df = pd.read_csv("/Users/sindhugunda/Documents/Information Technology/Year 3/CST3990/Final-Year-Project/Code/multimodal_test_public.csv")

# Reading the testing dataset
test_df = pd.read_csv("/Users/sindhugunda/Documents/Information Technology/Year 3/CST3990/Final-Year-Project/Code/traindata.csv")
...
train_df.dropna(subset=['clean_title'], inplace=True)
...
X_text_train = vectorizer.fit_transform(train_df['clean_title']).toarray()
...
max_tfidf_index = X_text_train.max(axis=0)[0].argmax()
print("Most important term:", feature_names[max_tfidf_index])
...
# test_df['clean_title'].dropna('', inplace=True)
# test_df['title'].dropna('', inplace=True)
test_df.dropna(subset=['clean_title'], inplace=True)
...
# GCN Model with two hidden layers
...
def evaluate(model, X, y, edge_index, criterion, device, mask):
    ...
    with torch.no_grad():
        ...
        acc = accuracy_score(y_true, y_pred)  # Calculate accuracy here
        # Detailed classification report
        report = classification_report(y_true, y_pred, target_names=['Real', 'Fake'], zero_division=0, output_dict=False)
        print(report)
        print(feature_names[:10])
        return loss.item(), acc, y_true, y_pred

# Integrate this updated evaluate function into your training and testing loops
for epoch in range(100):
    ...
    if epoch % 10 == 0:  # Evaluate every 10 epochs
        test_loss, test_accuracy, y_true, y_pred = evaluate(model, X_test_tensor, y_test_tensor, edge_index, criterion, device, test_mask)
        print(f'Test Loss: {test_loss:.4f}, Test Accuracy: {test_accuracy:.4f}')

# Final evaluation to display detailed metrics
test_loss, test_accuracy, y_true, y_pred = evaluate(model, X_test_tensor, y_test_tensor, edge_index, criterion, device, test_mask)
...
def explain_prediction(text):
    ...
    # Pairing terms with their scores and sorting them
    terms_scores = sorted(zip(feature_names, tfidf_scores), key=lambda x: x[1], reverse=True)

    # Selecting the top 5 features
    top_terms = terms_scores[:5]

    # Checking if the scores are all zero
    if not any(score > 0 for _, score in top_terms):
        return "No distinctive words found based on the current TF-IDF model."

    explanation = [{'term': term, 'tfidf': round(score, 2)} for term, score in top_terms if score > 0]
    return explanation
...
def predict():
    if request.method == 'POST':
        ...
        return jsonify({'prediction': prediction, 'reasons': explanation})
    else:
